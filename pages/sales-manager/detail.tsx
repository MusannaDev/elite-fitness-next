...
const SalesManagerDetail: NextPage = ({ initialComment, ...props }: any) => {
	const device = useDeviceDetect();
	const router = useRouter();
	const user = useReactiveVar(userVar);
	const [salesManagerId, setSalesManagerId] = useState<string | null>(null);
	const [salesManager, setSalesManager] = useState<Member | null>(null);
	const [commentInquiry, setCommentInquiry] = useState<CommentsInquiry>(initialComment);
	const [salesManagerComments, setSalesManagerComments] = useState<Comment[]>([]);
	...
	const {
		...
		variables: { input: salesManagerId },
		skip: !salesManagerId,
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			setSalesManager(data?.getMember);
			setCommentInquiry({
				...commentInquiry,
	...
	const createCommentHandler = async () => {
		try {
			...
			if (user._id === salesManagerId) throw new Error('Cannot write a review for yourself');
	...
	if (device === 'mobile') {
		return <div>SALES MANAGER DETAIL PAGE MOBILE</div>;
	} else {
		return (
			<Stack className={'sales-manager-detail-page'}>
				<Stack className={'container'}>
					<Stack className={'sales-manager-info'}>
						<img
							src={
					...
					<Stack className={'review-box'}>
						...
						{commentTotal !== 0 && (
							<Stack className={'review-wrap'}>
								...
								{salesManagerComments?.map((comment: Comment) => (
...
SalesManagerDetail.defaultProps = {
	initialComment: {
		page: 1,
		limit: 5,
		sort: 'createdAt',
...
export default withLayoutBasic(SalesManagerDetail);