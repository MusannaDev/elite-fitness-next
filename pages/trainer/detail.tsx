...
import ReviewCard from '../../libs/components/trainer/ReviewCard';
...
const TrainerDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	...
	const [trainerComments, setTrainerComments] = useState<Comment[]>([]);
	...
	const createCommentHandler = async () => {
		try {
			if (!user._id) throw new Error(Messages.error2);
			if (user._id === trainerId) throw new Error('Cannot write a review for yourself');
	...
	if (device === 'mobile') {
		return <div>TRAINER DETAIL PAGE MOBILE</div>;
	} else {
		return (
			<Stack className={'trainer-detail-page'}>
				<Stack className={'container'}>
					<Stack className={'trainer-info'}>
						<img
							src={
								trainer?.memberImage
					...
					<Stack className={'review-box'}>
						...
						{commentTotal !== 0 && (
							<Stack className={'review-wrap'}>
								...
								{trainerComments?.map((comment: Comment) => {
