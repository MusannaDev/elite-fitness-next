...
import Review from '../../libs/components/equipments/Review';
...
import { GET_EQUIPMENTS, GET_EQUIPMENT } from '../../apollo/user/query';
import { T } from '../../libs/types/common';
import { Direction, Message } from '../../libs/enums/common.enum';
import { CREATE_COMMENT, LIKE_TARGET_EQUIPMENT } from '../../apollo/user/mutation';
...
const EquipmentDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getEquipments?.list) setDestinationEquipments(data?.getEquipments?.list);
		},
	});
	...
	const commentPaginationChangeHandler = async (event: ChangeEvent<unknown>, value: number) => {
		...
		const addToCartHandler = async () => {
			try {
				if (!equipment?._id || !equipment?.equipmentPrice) return;
				addBasketItem({
					_id: equipment._id,
	...
	if (device === 'mobile') {
		...
		return (
			<div id={'equipment-detail-page'}>
				<div className={'container'}>
					<Stack className={'equipment-detail-config'}>
						<Stack className={'equipment-info-config'}>
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									<Typography className={'title-main'}>{equipment?.equipmentName}</Typography>
									<Stack className={'top-box'}>
										<Typography className={'city'}>{equipment?.equipmentLocation}</Typography>
										<Stack className={'divider'}></Stack>
										<Typography className={'brand'}>{equipment?.equipmentBrand}</Typography>
										<Stack className={'divider'}></Stack>
										...
										<Typography className={'date'}>
											{moment().diff(equipment?.createdAt, 'days')} days ago
										</Typography>
									</Stack>
						...
						<Stack className={'equipment-desc-config'}>
										...
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												<Box component={'div'} className={'info'}>
													...
													<Typography className={'data'}>${formatterStr(equipment?.equipmentPrice)}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Brand</Typography>
													<Typography className={'data'}>{equipment?.equipmentBrand}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Material</Typography>
													<Typography className={'data'}>{equipment?.equipmentMaterial}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
											...
											<Stack className={'right'}>
												<Box component={'div'} className={'info'}>
													...
													<Typography className={'data'}>{equipment?.equipmentCategory}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Location</Typography>
													<Typography className={'data'}>{equipment?.equipmentLocation}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
							...
							<Stack className={'right-config'}>
										...
										<img
											...
											src={
												equipment?.memberData?.memberImage
													? `${NEXT_PUBLIC_API_URL}/${equipment?.memberData?.memberImage}`
								...
								<Stack className={'info-box'}>
									...
									<textarea
										placeholder={'Hello, I am interested in this equipment...'}
									></textarea>
								</Stack>
								<Stack className={'info-box'}>
						...
						{destinationEquipments.length !== 0 && (
							<Stack className={'similar-equipments-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>Similar Equipment</Typography>
										<Typography className={'sub-title'}>Browse related fitness equipment</Typography>
									</Stack>
								...
								<Stack className={'cards-box'}>
									...
									>
										{destinationEquipments.map((equipment: Equipment) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={equipment.equipmentName}>
													<EquipmentBigCard
														equipment={equipment}
