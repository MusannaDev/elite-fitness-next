...
const SM = { color: '#d97706', bg: '#fef3c7', light: '#fffbf3' };

interface Props {
	setActiveStatus: (status: OrderStatus) => void;
}

const PendingOrders = ({ setActiveStatus }: Props) => {
	...
	const cancelHandler = async (orderId: string) => {
		try {
			const ok = await sweetConfirmAlert('Do you want to cancel this order?');
			if (!ok) return;
	...
	const confirmHandler = async (orderId: string) => {
		try {
			const ok = await sweetConfirmAlert('Confirm and proceed to payment?');
			if (!ok) return;
			const input: OrderUpdate = { orderId, orderStatus: OrderStatus.CONFIRMED };
			await updateOrder({ variables: { input } });
			await sweetTopSmallSuccessAlert('Order confirmed! 🎉', 800);
			setActiveStatus(OrderStatus.CONFIRMED);
		} catch (err: any) {
			sweetMixinErrorAlert(err.message);
		}
	};
	...
	return (
		<Stack gap="14px" className="os-pending-list">
			{orders.map((order: Order) => (
				<div key={order._id} className="oc-card oc-card--pending" style={{ borderLeftColor: SM.color }}>
					...
					<div className="oc-card-footer">
						...
						<div className="oc-btn-group">
							...
							<Button className="btn-oc-confirm" startIcon={<CheckCircleIcon />}
								onClick={() => confirmHandler(order._id)}>Confirm</Button>
							<AddItemMenuButton />
						</div>
					</div>
				</div>
